...
st.write("Upload your PDF and a TrueType/OpenType font file (e.g., times.ttf) to automatically add signature panels.")

# Streamlit file uploaders
pdf_file = st.file_uploader("Upload PDF File", type=['pdf'])
font_file = st.file_uploader("Upload Font File (e.g., times.ttf)", type=['ttf', 'otf'])

if pdf_file and font_file:
    if st.button("Generate Signature Panels"):
        with st.spinner("Processing PDF..."):
            
            # --- Handle Font File ---
            # PyMuPDF requires a file path to load custom fonts, so we save the uploaded font to a temporary file
